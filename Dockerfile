...
RUN yum -y install bind-utils openssl unzip findutils net-tools nc jq which iptables libmnl krb5-workstation krb5-libs && yum clean all && rm -rf /var/cache/yum || true && exit 0
RUN apt-get update; apt-get -y install bind-utils openssl unzip findutils net-tools nc jq which iptables iproute && rm -rf /var/lib/apt/lists/* || true && exit 0
...
RUN confluent-hub install --no-prompt confluentinc/kafka-connect-gcp-spanner:1.0.4
